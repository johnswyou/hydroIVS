...
# %=========================================================================
#-------------------------------------------------------------------------------
MI_KNN <- function(Y,X,k,psi.nn,psi.nsamp) {

  # Computes the K nearest neighbour (KNN)-based Shannon Mutual Information

  # get max distances
  maxdists = knn_max_dist(cbind(Y,X),k+1)

  # find points within max distance in projected space (add small noise
  # in case some discrete inputs)
  smallnoise = 10^-15
  npY = knn_radius(Y,maxdists + smallnoise)
  npX = knn_radius(X,maxdists + smallnoise)
  psi.both = psigamma(cbind(npY,npX))

  MI_nn = psi.nn + psi.nsamp - mean(rowSums(psi.both)) # MI (Eq. 8 in Vlachos and Kugiumtzis [2010])

  MI_nn = max(0,Re(MI_nn))
  return(MI_nn)
}
#-------------------------------------------------------------------------------
CMI_KNN <- function(Y,X,Z,k,psi.nn,psi.nsamp) {
  ...
  # get max distances
  maxdists = knn_max_dist(cbind(Y,X,Z),k+1)

  # find points within max distance in projected space (add small noise
  # in case some discrete inputs)
  smallnoise = 10^-15
  npZ = knn_radius(Z,maxdists + smallnoise)
  npXZ = knn_radius(cbind(X,Z),maxdists + smallnoise)
  npYZ = knn_radius(cbind(Y,Z),maxdists + smallnoise)
  npY = knn_radius(Y,maxdists + smallnoise)
  psi.npZ = psigamma(npZ)
  psi.npXZ = psigamma(npXZ)
  psi.npYZ = psigamma(npYZ)
  psi.npY = psigamma(npY)
  psi.three = cbind(psi.npYZ,psi.npXZ,-psi.npZ)
  psi.two = cbind(psi.npY,psi.npXZ)

  CMI_nn = psi.nn - mean(rowSums(psi.three)) # CMI (Eq. 10 in Vlachos and Kugiumtzis [2010])
  MIp_nn = psi.nn + psi.nsamp - mean(rowSums(psi.two)) # projected MI (see Eq. 10-12 in Vlachos and Kugiumtzis [2010])
  ...
  for(iter.1 in 1:max.iter) {
    if(n.selected > 0) {
      ...
      for(current.inp in 1:n.inputs) {
        ...
        # Compute CMI
        CMI_MIp_nn = CMI_KNN(y, z, z.in, k, psik, psiN)
      ...
      for(current.inp in 1:n.inputs) {
        z <- x[,current.inp]
        CMI[current.inp] <- MI_KNN(y, z, k, psik, psiN)
        nfevals.2 <- nfevals.2 + 1
        Input[current.inp] <- input.tracker[current.inp]
      }
    }
